...
#pragma mark - Rewrite the superclass method
+ (void)initialize {
...
/** 获取所有属性，包含主键 Primary Key */
+ (NSDictionary *)getAllProperties {
    NSDictionary *dict = [self.class getPropertys];
    
    NSMutableArray *proNames = [NSMutableArray array];
    NSMutableArray *proTypes = [NSMutableArray array];
    [proNames addObject:kPrimaryKeyId];
    [proTypes addObject:[NSString stringWithFormat:@"%@ %@",SQLINTEGER,kPrimaryKeyId]];
...
- (BOOL)saveOrUpdate {
    id primaryValue = [self valueForKey:kPrimaryKeyId];
...
- (BOOL)saveOrUpdateByColumnName:(NSString*)columnName AndColumnValue:(NSString*)columnValue {
    ...
    if (record) {
        id primaryValue = [record valueForKey:kPrimaryKeyId]; //取到了主键PK
        if ([primaryValue intValue] <= 0) {
            return [self save];
        }else{
...
- (BOOL)save {
    ...
    for (int i = 0; i < self.columeNames.count; i++) {
        ...
        if ([proname isEqualToString:kPrimaryKeyId]) {
            continue;
        }
...
- (BOOL)update {
    ...
    [dbManager.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        //删除最后那个逗号
        [keyString deleteCharactersInRange:NSMakeRange(keyString.length - 1, 1)];
        NSString *sql = [NSString stringWithFormat:@"UPDATE %@ SET %@ WHERE %@ = ?;", tableName, keyString, kPrimaryKeyId];
...
+ (BOOL)updateObjects:(NSArray *)array {
    ...
    [dbManager.dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        for (XLDBModel *model in array) {
            NSString *tableName = NSStringFromClass(model.class);
            id primaryValue = [model valueForKey:kPrimaryKeyId];
            if (!primaryValue || primaryValue <= 0) {
                res = NO;
                *rollback = YES;
                return;
...
- (BOOL)deleteObject {
    ...
    [dbManager.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        NSString *sql = [NSString stringWithFormat:@"DELETE FROM %@ WHERE %@ = ?",tableName, kPrimaryKeyId];
...
+ (instancetype)findByPK:(int)inPk {
    NSString *condition = [NSString stringWithFormat:@"WHERE %@=%d", kPrimaryKeyId, inPk];
