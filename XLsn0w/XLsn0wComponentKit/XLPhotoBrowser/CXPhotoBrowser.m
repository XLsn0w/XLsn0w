...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    // Status bar
    if (self.edgesForExtendedLayout && UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone) {
...
- (void)currentPageDidUpdatedWithIndex:(NSUInteger)index
{
    ...
    if (_shouldUseDefaultUINavigationBar) {
        self.title = [NSString stringWithFormat:@"%lu of %lu", (unsigned long)index+1, (unsigned long)_photoCount];
    }
}
