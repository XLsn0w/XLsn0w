...
///单例 singleton
+ (instancetype)shared {
    static XLsn0wKit *singleton = nil;
...
+ (NSMutableAttributedString *)firstString:(NSString *)firstString
    ...
    NSMutableAttributedString * firstAttributedString = [[NSMutableAttributedString alloc] initWithString:[XLsn0wKit isStringNullOrEmpty:firstString]];
    ...
    NSMutableAttributedString * secondAttributedString = [[NSMutableAttributedString alloc] initWithString:[XLsn0wKit isStringNullOrEmpty:secondString]];
...
- (void)imageSavedToPhotosAlbum:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo {
    if (!error) {
        [XLsn0wKit xl_showTipText:@"成功保存到相册"];
    } else {
        [XLsn0wKit xl_showTipText:@"保存图片失败"];
    }
}
...
{
    [XLsn0wKit showTipsWithView:inView labelText:labelText showTime:1.5];
}
...
+ (void)showNotReachabileTips {
    [XLsn0wKit showTipsWithHUD:@"与服务端连接已断开,请检查您的网络连接是否正常"];
}
 ...
 ****************************************************/
- (XLsn0wKit *)chineseCalendarDate {
    XLsn0wKit *lunarCalendar = [[XLsn0wKit alloc] init];
