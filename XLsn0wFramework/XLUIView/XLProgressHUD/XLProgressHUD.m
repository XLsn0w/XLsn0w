...
@property (nonatomic, strong) ProgressAnimatedView *ringView;
@property (nonatomic, strong) ProgressAnimatedView *backgroundRingView;
...
+ (XLProgressHUD *)sharedView {
    static dispatch_once_t once;
    
    static XLProgressHUD *sharedView;
...
+ (void)setDefaultStyle:(ProgressHUDStyle)style {
    [self sharedView].defaultStyle = style;
}

+ (void)setDefaultMaskType:(ProgressHUDMaskType)maskType {
    [self sharedView].defaultMaskType = maskType;
}

+ (void)setDefaultAnimationType:(ProgressHUDAnimationType)type {
...
+ (void)showWithMaskType:(ProgressHUDMaskType)maskType {
    ProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
...
+ (void)showProgress:(float)progress maskType:(ProgressHUDMaskType)maskType {
    ProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showInfoWithStatus:(NSString*)status maskType:(ProgressHUDMaskType)maskType {
    ProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showSuccessWithStatus:(NSString*)status maskType:(ProgressHUDMaskType)maskType {
    ProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showErrorWithStatus:(NSString*)status maskType:(ProgressHUDMaskType)maskType {
    ProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showImage:(UIImage*)image status:(NSString*)status maskType:(ProgressHUDMaskType)maskType {
    ProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
#pragma clang diagnostic pop
	...
	if(string) {
        if(self.defaultAnimationType == SVProgressHUDAnimationTypeFlat) {
            IndefiniteAnimatedView *indefiniteAnimationView = (IndefiniteAnimatedView*)self.indefiniteAnimatedView;
...
- (void)updateMask {
    ...
    switch (self.defaultMaskType) {
        ...
        case SVProgressHUDMaskTypeGradient:{
            RadialGradientLayer *layer = [RadialGradientLayer layer];
...
- (void)positionHUD:(NSNotification*)notification {
    ...
    if(notification) {
        ...
        __weak XLProgressHUD *weakSelf = self;
        [UIView animateWithDuration:animationDuration
                              delay:0
                            options:UIViewAnimationOptionAllowUserInteraction
                         animations:^{
                             __strong XLProgressHUD *strongSelf = weakSelf;
                             if(strongSelf) {
...
- (UIView*)indefiniteAnimatedView {
    ...
    if(self.defaultAnimationType == SVProgressHUDAnimationTypeFlat){
        ...
        if(_indefiniteAnimatedView && ![_indefiniteAnimatedView isKindOfClass:[IndefiniteAnimatedView class]]){
            [_indefiniteAnimatedView removeFromSuperview];
            _indefiniteAnimatedView = nil;
        }
        
        if(!_indefiniteAnimatedView){
            _indefiniteAnimatedView = [[IndefiniteAnimatedView alloc] initWithFrame:CGRectZero];
        }
        
        // Update styling
        IndefiniteAnimatedView *indefiniteAnimatedView = (IndefiniteAnimatedView*)_indefiniteAnimatedView;
...
- (ProgressAnimatedView*)ringView {
    if(!_ringView) {
        _ringView = [[ProgressAnimatedView alloc] initWithFrame:CGRectZero];
    }
...
- (ProgressAnimatedView*)backgroundRingView {
    if(!_backgroundRingView) {
        _backgroundRingView = [[ProgressAnimatedView alloc] initWithFrame:CGRectZero];
...
- (void)setDefaultAnimationType:(ProgressHUDAnimationType)animationType {
